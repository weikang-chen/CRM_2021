...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#qx").click(function () {
			...
			$("#modalActivityBody").on("click",$("input[name=xz]"),function () {
				$("#qx").prop("checked",$("input[name=xz]").length == $("input[name=xz]:checked").length);
			})

		})
		
		//线索详细页面加载完毕后，显示市场活动列表
		showActivityList();

		//关联市场活动的模态窗口的搜索绑定事件，按回车搜索，并展示结果
		$("#aname").keydown(function (event) {
			if(event.keyCode == 13){

				$.ajax({
					url: "workbench/clue/getActivityByNameAndClueId.do",
					data: {
						"name":$.trim($("#aname").val()),
						"clueId":"${c.id}"
					},
		...
		$("#bundBtn").click(function () {
			...
			if($xz.length == 0){
				...
				//确认框
				if(confirm("确认关联吗？")){
					//url:workbench/clue/bund.do?cid=xxx&aid=xxx&aid=xxx...
					...
					$.ajax({
						...
						success: function (data) {
							if(data.success){
								...
								$("#anameSearchForm")[0].reset();
								$("#modalActivityBody").html("");
							}else {
								alert("关联失败！");
							}
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form id="anameSearchForm" class="form-inline" role="form">
